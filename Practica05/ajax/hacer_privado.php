...
if (isset($_POST['archivo'])) {
    ...
    try {
        $db = getDbConnection();
        $sqlCmd = "UPDATE archivos SET es_publico = ? WHERE id = ?";
        $stmt = $db->prepare($sqlCmd);
        $stmt->execute([0, $idArchivo]);

        $db = getDbConnection();
        $sqlCmd = "INSERT INTO archivos_log_general (archivo_id, usuario_id, fecha_hora, accion_realizada, ip_realiza_operacion)
            VALUES (?, ?, ?, ?, ?)";
        $stmt = $db->prepare($sqlCmd);
        $stmt->execute([$idArchivo, $usuario_id, $date, 'PRIVATIZAR', $REQUEST_IP_ADDRESS]);
    } catch (Exception $e) {
    ...
    $resObj["mensaje"] = "Archivo privatizado exitosamente.";
    echo json_encode($resObj);
}